        ...
        if (jl_is_toplevel_only_expr(expr) || e->head == jl_const_sym ||
                ...
                if (fe_mod->istopmod && !strcmp(jl_symbol_name(fe_sym), "getproperty") && jl_is_symbol(s)) {
                    if (eager_resolve || jl_binding_resolved_p(me_mod, me_sym)) {
                        ...
                        if (b) {
                            ...
                            jl_value_t *v = jl_get_value_if_const(bpart);
                            if (v && jl_is_module(v))
            ...
            if (e->head == jl_call_sym && nargs > 0 &&
                ...
                if (jl_binding_resolved_p(fe_mod, fe_sym)) {
                    ...
                    if (jl_get_value_if_const(jl_get_binding_partition(b, jl_current_task->world_age)) == jl_builtin_tuple) {
                        size_t j;
                        for (j = 1; j < nargs; j++) {
                                                 ...
                                                 jl_binding_t *bnd)
{
    ...
    assert(name);
    if (bnd) {
        gf = jl_get_value_if_const(jl_get_binding_partition(bnd, jl_current_task->world_age));
        if (!gf) {
            bp = (_Atomic(jl_value_t*)*)&jl_declare_constant(bnd, module, name)->restriction;
        }
    } else {
        assert(bp);
