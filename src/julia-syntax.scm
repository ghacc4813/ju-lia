              ...
              (cond
                ...
                (else
                 (let ((rr (make-ssavalue)))
                   (expand-forms `(let (block)
                         (block
                           (= ,rr (local ,arg))
                           ,.(map (lambda (v) `(,(car e) (globalref (thismodule) ,v) ,v)) (lhs-vars (cadr arg)))
                           ,rr)))))))
...
(define (lower-tuple-assignment lhss x (wrap identity))
  (let ((t (make-ssavalue)))
    `(block
      (= ,t ,x)
      ,@(let loop ((lhs lhss)
